...
def main():
    ...
    # --- Custom CSS for Blue Slider (Request 2) ---
    st.sidebar.markdown("""
        <style>
        /* Target the DTI slider track fill */
        .stSlider > div > div > div:nth-child(2) {
            background-color: #1E90FF !important; /* Blue color */
        }
        /* Target the DTI slider handle */
    ...
    # --- A. Input Sliders (Numerical Features) - Two Column Layout (Request 1) ---
    ...
    for i, feature in enumerate(ALL_SLIDER_FEATURES):
        ...
        # Split features across the two columns
        target_column = col_num1 if i < 7 else col_num2 # 7 in col1, 6 in col2
    ...
    # --- B. Select Boxes (Target-Encoded Features) - Two Column Layout (Request 1) ---
    st.sidebar.subheader("2. Categorical Features")
    
    col_cat1, col_cat2 = st.sidebar.columns(2)
    
    # The TE_MAPPINGS dictionary has 7 categories (gender, marital_status, education_level, employment_status, loan_purpose, grade_subgrade, grade_letter)
    for i, (original_col, mapping) in enumerate(TE_MAPPINGS.items()):
        te_feature_name = f'TE_{original_col}'
        
        target_column = col_cat1 if i < 4 else col_cat2 # 4 in col1, 3 in col2
        
        selected_option = target_column.selectbox(
