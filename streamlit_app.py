...
# B. FINAL FEATURES LIST (FIXED: Reduced to the 20 features the model actually expects)
FINAL_FEATURES = [
    'annual_income', 
...
# C. FEATURE CATEGORY LISTS (SIMPLIFIED: Only defining lists used for UI creation)
NUMERICAL_FEATURES = [
...
# Your previous binary list contained many scaled numerical features.
# To keep UI simple, let's assume ALL non-TE features are generated by the combined list for sliders.
# You must manually enter all non-TE features into the ALL_SLIDER_FEATURES to avoid missing inputs.

# D. TARGET ENCODING MAPPINGS (FIXED: Added 'grade_letter' mapping since the TE_grade_letter feature is required)
TE_MAPPINGS = {
    ...
    'grade_letter': {'A': 0.954, 'B': 0.935, 'C': 0.845, 'D': 0.718, 'E': 0.655, 'F': 0.625, 'G': 0.580} # Estimated values needed for the TE_grade_letter feature
}
...
def predict_and_explain(model, input_data):
    """Makes a prediction and calculates SHAP values. (FIXED: Single SHAP call, correct indexing)"""
    
    # CRITICAL FIX: Reorder columns to match the trained model's 20-feature order
    ...
    # [0] indexes the single sample's values from the (1, 20) matrix
...
def main():
    ...
    if st.sidebar.button("Analyze Loan Risk", type="primary"):
        ...
        try:
            # This is the line that will now succeed, as input_df and FINAL_FEATURES match the model
